...
/// </list>
/// </remarks>
public class MatterDocumentActivityUserTo : IEquatable<MatterDocumentActivityUserTo>, IComparable<MatterDocumentActivityUserTo>
{
    ...
    /// </code>
    /// </example>
    [Required(ErrorMessage = "Matter ID is required for destination matter association.")]
    ...
    /// </code>
    /// </example>
    [Required(ErrorMessage = "User ID is required for destination user attribution.")]
    ...
    /// Console.WriteLine($"Transfer is {transferTo.AgeDays} days old");
    /// </code>
    /// </example>
    [NotMapped]
    ...
    public string TransferDescription =>
        ...
        $"to '{Matter?.Description ?? "Unknown Matter"}' by {User?.Name ?? "Unknown User"} at {CreatedAt:yyyy-MM-dd HH:mm} UTC";
    ...
    /// </remarks>
    public bool Equals(MatterDocumentActivityUserTo? other)
    {
    ...
    public override bool Equals(object? obj) => Equals(obj as MatterDocumentActivityUserTo);
    ...
    /// <summary>
    /// Compares the current MatterDocumentActivityUserTo with another MatterDocumentActivityUserTo for ordering purposes.
    /// </summary>
    /// <param name="other">The MatterDocumentActivityUserTo to compare with the current MatterDocumentActivityUserTo.</param>
    /// <returns>
    ...
    /// </code>
    /// </example>
    public int CompareTo(MatterDocumentActivityUserTo? other)
    {
    ...
    public static bool operator ==(MatterDocumentActivityUserTo? left, MatterDocumentActivityUserTo? right) =>
        EqualityComparer<MatterDocumentActivityUserTo>.Default.Equals(left, right);
    ...
    public static bool operator !=(MatterDocumentActivityUserTo? left, MatterDocumentActivityUserTo? right) => !(left == right);
    ...
    public override string ToString() =>
        $"Document {MatterDocumentActivity?.Activity ?? "TRANSFERRED"} to Matter '{Matter?.Description ?? MatterId.ToString()[..8]}' " +
    ...
    public IEnumerable<ValidationResult> Validate()
    {
        ...
        // Validate activity appropriateness (requires destination user for complete audit trail)
        if (MatterDocumentActivity == null) return results;
        var hasToUser = UserId != Guid.Empty;
        if (!Common.MatterDocumentActivityValidationHelper.IsActivityAppropriateForContext(
                MatterDocumentActivity.Activity, false, hasToUser))
        {
            results.Add(new ValidationResult(
