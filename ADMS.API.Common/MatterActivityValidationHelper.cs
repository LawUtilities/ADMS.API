...
public static class MatterActivityValidationHelper
{
    ...
    /// </remarks>
    public const int MaxActivitySuggestions = 10;
    ...
    /// </code>
    /// </example>
    public static bool IsActivityAppropriateForMatterStatus(
        string? activity,
        bool isArchived,
        bool isDeleted)
    {
        ...
        return (normalizedActivity, isArchived, isDeleted) switch
        {
            // Cannot archive an already archived matter
            ("ARCHIVED", true, _) => false,

            // Cannot unarchive a non-archived matter
            ("UNARCHIVED", false, _) => false,

            // Cannot delete an already deleted matter
            ("DELETED", _, true) => false,

            // Deleted matters can only be restored
            (_, _, true) when normalizedActivity != "RESTORED" => false,
        ...
        bool allowDuplicates = true)
    {
        ...
        // Activities that typically should only happen once
        string[] singleOccurrenceActivities = ["CREATED", "DELETED"];

        if (!allowDuplicates || singleOccurrenceActivities.Contains(normalizedActivity))
        {
        ...
        int maxSuggestions = MaxActivitySuggestions)
    {
        ...
        {
            // Return most common activities if no input provided
        ...
        {
            // Fill remaining slots with most common activities
            var remainingCount = maxSuggestions - similarActivities.Count;
            var commonActivities = AllowedActivities
                .Except(similarActivities)
                .Take(remainingCount);

            similarActivities.AddRange(commonActivities);
        }
    ...
    public static IReadOnlyDictionary<string, object> GetValidationStatistics()
    {
        return new Dictionary<string, object>
        {
            ...
            ["ActivityCategories"] = new Dictionary<string, int>
            {
                ...
                ["Archive"] = GetActivitiesByCategory("archive").Count(),
                ["Access"] = GetActivitiesByCategory("access").Count()
            }
    ...
    public static string GenerateValidationReport(string? activity)
    {
        ...
        report.AppendLine($"Matter Activity Validation Report for: '{activity ?? "<null>"}'");
        ...
        var categories = new[] { "lifecycle", "archive", "access" };
        foreach (var category in categories)
        {
            var categoryActivities = GetActivitiesByCategory(category);
            report.AppendLine($"  {char.ToUpperInvariant(category[0])}{category[1..]}: {string.Join(", ", categoryActivities)}");
        }
