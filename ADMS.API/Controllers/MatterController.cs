...
namespace ADMS.API.Controllers
{
    ...
    public class MatterController : ControllerBase
    {
            ...
            Guid id)
        {
            try
            {
                if (!await _admsRepository.CheckMatterExists(id))
                {
                    return NotFound("Matter not found");
                }

                Matter? matter = await _admsRepository.GetMatterAsync(id, false);
            ...
            {
                string errorMessage = $"An error occured while deleting matter with id: {id}";
                _logger.LogCritical(message: errorMessage, args: exception);
                return BadRequest("An error occured");
            }
        }
            ...
            bool includeDocuments = false)
        {
            try
            {
                ...
                Matter? matter = await _admsRepository.GetMatterAsync(id, includeDocuments);
                if (includeDocuments)
                {
            ...
            {
                string errorMessage = $"An error occured while retrieving Matter with id: {{id}}";
            ...
            Guid id)
        {
            try
            {
                ...
                return Ok(await _admsRepository.GetMDAUFromHistoryAsync(id, Guid.Empty));
            }
            catch (Exception exception)
            {
                string errorMessage = $"An error occured while retrieving Matter with id: {{id}}";
            ...
            Guid id)
        {
            try
            {
                ...
                return Ok(await _admsRepository.GetMDAUToHistoryAsync(id, Guid.Empty));
            }
            catch (Exception exception)
            {
                string errorMessage = $"An error occured while retrieving Matter with id: {{id}}";
        ...
        /// <response code="400">An error occured</response>
        /// <response code="404">Matter not found</response>
        [HttpGet("{id}/MatterHistoryExists")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
            ...
            Guid id)
        {
            try
            {
                ...
                bool result = await _admsRepository.DoesMatterHistoryExists(id);

                return Ok(new JsonResult(result));
            }
            catch (Exception exception)
            {
                string errorMessage = $"An error occured while retrieving Matter history with id: {{id}}";
                _logger.LogCritical(errorMessage, exception);
                return BadRequest("An error occured");
            }
        }
            ...
            Guid id)
        {
            try
            {
                ...
                Matter matterWithHistory = await _admsRepository.GetMatterWithHistoryByIdAsync(id);
            ...
            Guid id)
        {
            try
            {
                ...
                var restorationResult = await _admsRepository.RestoreMatterAsync(id);
            ...
            MatterForUpdateDto matter)
        {
            try
            {
                ...
                Matter? matterEntity = await _admsRepository.GetMatterAsync(id, false);
            ...
            {
                string errorMessage = $"An error occured while updating Matter with id: {id}";
                _logger.LogCritical(message: errorMessage, args: exception);
                return BadRequest("An error occured");
            }
        }
            ...
            CancellationToken cancelToken)
        {
            try
            {
                ...
                if (matter == null || matter.Id == Guid.Empty)
                    return BadRequest($"Could not retrieve matter with id: {id}");
                ...
                Document? newDocument = await _admsRepository.AddDocumentAsync(id, documentToCreate);
        ...
        private void AddCustomDocumentProperties(string tempFile, string fileName, Guid documentId, Guid revisionId, Guid matterId, DocumentType docType)
        {
            try
            {
        ...
        private void AddOrUpdateDocumentProperty(dynamic document, string propertyName, string propertyValue, DocumentType docType)
        {
            try
            {
        ...
        /// <param name="propertyValue">specific value to add</param>
        /// <param name="docType">type of document to add property to</param>
        private void SetDocumentProperty(dynamic document, string propertyName, string propertyValue, DocumentType docType)
        {
            try
            {
                switch (docType)
