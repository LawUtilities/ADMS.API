...
namespace ADMS.API.Services
{
        ...
        IMapper mapper) : IAdmsRepository
    {
        ...
        public async Task<ICollection<MatterDocumentActivityUserFromDto>> GetMDAUFromHistoryAsync(
            Guid matterId,
            Guid documentId)
        {
            try
            {
                ...
                return _mapper.Map<ICollection<MatterDocumentActivityUserFromDto>>(histories);
            }
            catch (Exception exception)
            {
        ...
        /// <param name="documentId">document to retrieve data for</param>
        /// <returns>collection of matter document activity user histories</returns>
        public async Task<ICollection<MatterDocumentActivityUserToDto>> GetMDAUToHistoryAsync(
            Guid matterId,
            Guid documentId)
        {
            try
            {
                ...
                if (matterId != Guid.Empty)
                {
                    if (documentId != Guid.Empty)
                    {
                        histories = await _context
                            ...
                            .Where(mdaut =>
                                mdaut.MatterId == matterId &&
                                mdaut.DocumentId == documentId)
                            .Include(md => md.Matter)
                            .Include(md => md.MatterDocumentActivity)
                            .Include(md => md.Document)
                            .Include(md => md.User)
                ...
                return _mapper.Map<ICollection<MatterDocumentActivityUserToDto>>(histories);
            }
            catch (Exception exception)
            {
                string errorMessage = string.Empty;
